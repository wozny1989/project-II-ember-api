...
module.exports = {
  ...
  emberizeJSON: function (model, records, associations, sideload) {
    ...
    var documentIdentifier = plural
      ? pluralize(model.globalId)
      : model.globalId;
    ...
    if (sideload) {
      ...
      forEach(associations, function (assoc) {
        ...
        if (assoc.type === 'collection') {
          assocName = pluralize(
            camelCase(sails.models[assoc.collection].globalId)
          );
        } else {
  ...
  parseValues: function (req, model) {
    ...
    if (blacklist && !isArray(blacklist)) {
      throw new Error(
        'Invalid `req.options.values.blacklist`. Should be an array of strings (parameter names.)'
      );
    }
  ...
  parseLimit: function (req) {
    ...
    var limit =
      req.param('limit') ||
      (typeof req.options.limit !== 'undefined'
        ? req.options.limit
        : DEFAULT_LIMIT);
    if (limit) {
      limit = +limit;
  ...
  parseSkip: function (req) {
    ...
    var skip =
      req.param('skip') ||
      (typeof req.options.skip !== 'undefined'
        ? req.options.skip
        : DEFAULT_SKIP);
    if (skip) {
      skip = +skip;
